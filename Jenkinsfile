            ...
            steps {
                ...
                script {
                    env.RUN_ID = powershell(
                        script: "$resultsPath = Join-Path $env:WORKSPACE 'results.json'; if (!(Test-Path $resultsPath)) { throw 'results.json not found'; } (Get-Content -Raw $resultsPath | ConvertFrom-Json).run_id",
                        returnStdout: true
                    ).trim()
                }
